...
export const pernDemoAuth = betterAuth({
  secret: pernDemoConfig.auth.secret,
  baseURL: pernDemoConfig.auth.baseUrl,
  basePath: pernDemoConfig.auth.basePath,
  trustedOrigins: pernDemoConfig.auth.trustedOrigins,
  database: prismaAdapter(prisma, {
    provider: "postgresql",
  }),
  socialProviders: {
    google: {
      clientId: pernDemoConfig.auth.socialProviders.google.id,
      clientSecret: pernDemoConfig.auth.socialProviders.google.secret,
      mapProfileToUser: (profile) => ({ name: profile.name }),
    },
    github: {
      clientId: pernDemoConfig.auth.socialProviders.github.id,
      clientSecret: pernDemoConfig.auth.socialProviders.github.secret,
      mapProfileToUser: (profile) => ({ name: profile.name }),
    },
  },
  emailAndPassword: {
